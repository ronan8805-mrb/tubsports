...
export function RunnerRow({ runner, rank }: Props) {
  ...
  return (
    ...
    >
      {/* Main row */}
      <div
        className="px-3 py-3 cursor-pointer hover:bg-white/[0.02] transition-colors sm:px-4"
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {runner.draw != null && (
            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-md flex items-center justify-center text-[10px] sm:text-xs font-bold bg-sky-500/10 text-sky-400 border border-sky-500/20 shrink-0">
              {runner.draw}
            </div>
          )}
          ...
          {runner.silk_url && (
            <img
              src={runner.silk_url}
              alt=""
              className="w-7 h-7 sm:w-8 sm:h-8 rounded object-contain bg-white/10 shrink-0"
              loading="lazy"
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className={`font-semibold text-sm truncate ${isTopPick ? 'text-white' : 'text-gray-200'}`}>
                {runner.horse_name}
              </span>
              {rank === 1 && (
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] sm:text-[9px] font-bold uppercase tracking-wider bg-amber-500/20 text-amber-400 border border-amber-500/30 shrink-0">
                  TOP PICK
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              {runner.jockey && (
                <span className="text-[10px] text-gray-500 truncate max-w-[100px] sm:max-w-[120px]">
                  J: {runner.jockey}
                </span>
              )}
              {runner.trainer && (
                <span className="text-[10px] text-gray-500 truncate max-w-[100px] sm:max-w-[120px] hidden sm:inline">
                  T: {runner.trainer}
                </span>
              )}
            </div>
          ...
          {valueBadge && (
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border shrink-0 ${valueBadge.cls}`}>
              {valueBadge.text}
            </span>
          )}
        ...
        {/* Bottom: Stats row */}
        <div className="flex items-center gap-0 mt-2 ml-9 sm:ml-10">
          <div className="flex-1 text-center">
            <div className="text-xs sm:text-sm font-mono text-blue-400">
              {(runner.win_prob * 100).toFixed(1)}%
            </div>
            <div className="text-[9px] sm:text-[10px] text-gray-500">Win</div>
          </div>
          ...
          <div className="flex-1 text-center">
            ...
            <div className="text-[9px] sm:text-[10px] text-gray-500">Place</div>
          </div>

          <div className="w-px h-6 bg-gray-700/50" />

          <div className="flex-1 text-center">
            <div className={`text-xs sm:text-sm font-bold font-mono ${color}`}>
              {fractional}
            </div>
          ...
          <div className="flex-1 text-center">
            {bookieFrac ? (
              ...
              <>
                <div className="text-xs sm:text-sm font-mono text-gray-600">--</div>
                <div className="text-[9px] sm:text-[10px] text-gray-600">Bookie</div>
              </>
            )}
          </div>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="px-3 pb-4 pt-1 border-t border-gray-800/50 sm:px-4">
              ...
              <div className="bg-gray-800/50 rounded-lg p-3 mb-3">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  <div>
                    ...
                    <div className={`text-base sm:text-lg font-bold font-mono ${color}`}>{fractional}</div>
                    <div className="text-[10px] text-gray-500">({runner.fair_odds.toFixed(2)})</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Win Chance</div>
                    <div className="text-base sm:text-lg font-bold font-mono text-blue-400">{(runner.win_prob * 100).toFixed(1)}%</div>
                  ...
                  <div>
                    ...
                    <div className="text-base sm:text-lg font-bold font-mono text-emerald-400">{(runner.place_prob * 100).toFixed(1)}%</div>
                  ...
                  {runner.back_odds != null && (
                    <>
                      <div>
                        ...
                        <div className="text-base sm:text-lg font-bold font-mono text-white">{bookieFrac}</div>
                        <div className="text-[10px] text-gray-500">({runner.back_odds.toFixed(2)})</div>
                      </div>
                      <div>
                        ...
                        <div className="text-[10px] text-gray-500">
                          {runner.value_flag === 'VALUE'
                            ? 'Bookie offering more'
                            : runner.value_flag === 'SHORT'
                              ? 'Bookie offering less'
                              : 'Prices aligned'}
                        </div>
                      </div>
                    </>
